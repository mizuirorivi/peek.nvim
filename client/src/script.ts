...
addEventListener('DOMContentLoaded', () => {
  ...
  const pendingDirRequests = new Map<string, HTMLElement>();
  let rootDirPath = '';
      ...
      if (entry.isDir) {
        ...
        item.addEventListener('click', (e) => {
          ...
          if (isOpen && children.children.length === 0) {
            pendingDirRequests.set(entry.path, children);
  ...
  socket.onmessage = (event) => {
    ...
    switch (data.action) {
      ...
      case 'base':
        base.href = data.base;
        rootDirPath = data.base.replace(/\/$/, '');
        filesTree.innerHTML = '';
        break;
      case 'dirlist': {
        const container = pendingDirRequests.get(data.path);
        if (container) {
          renderFileTree(container, data.entries, 0);
  ...
  filesBtn.addEventListener('click', () => {
    ...
    if (opening) {
      ...
      if (filesTree.children.length === 0 && rootDirPath) {
        pendingDirRequests.set(rootDirPath, filesTree);
